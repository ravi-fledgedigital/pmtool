...
class OrderRepository
{
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/midtrans/saveOrderCancel.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        try {
            $logger->info('Order Cancel Save Start');
            $this->magentoOrderRepository->save($order);
        } catch (Exception $e) {
            $logger->info("Order Cancel Exception: " . $e->getMessage());
