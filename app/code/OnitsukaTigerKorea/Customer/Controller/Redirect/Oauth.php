...
namespace OnitsukaTigerKorea\Customer\Controller\Redirect;
...
use Seoulwebdesign\KakaoSync\Controller\Redirect\Oauth as SeoulwebdesignOauth;
use Seoulwebdesign\KakaoSync\Model\CustomerAuthorize;
use Seoulwebdesign\KakaoSync\Service\Kakao;

class Oauth extends SeoulwebdesignOauth
{
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/customer_oauth.log');
        $logger = new \Zend_Log();
        ...
        try {
            ...
            if ($tokenData) {
                ...
                $kakaoCustomerId = isset($userInfo['id']) ?: '';
                    ...
                    try {
                        ...
                        if ($customerLogin) {
                            ...
                            if ($sessionEmail) {
                                ...
                                $customerEmailLoad = $this->customerFactory->create()
                                    ->setWebsiteId($customerSession->getCustomer()->getWebsiteId())
                                    ->loadByEmail($email);
                                $customerEmail = $customerEmailLoad->getEmail();
                                ...
                                if (!$customerEmail) {
                                    ...
                                    $redirect = $objectManager->get(\Magento\Framework\Controller\Result\RedirectFactory::class);
                                    $resultRedirect = $redirect->create();
                                    $resultRedirect->setPath('customer/account/login');
                                    return $resultRedirect;
                                } else {
                                    $logger->info("else same customer");
                                    $logger->info("sessionEmail : " . $sessionEmail);
                                    ...
                                    if ($sessionEmail != $email) {
                                        ...
                                        $redirect = $objectManager->get(\Magento\Framework\Controller\Result\RedirectFactory::class);
                                        $messageManager->getMessages(true);
                                        $messageManager->addErrorMessage(__("A customer with the same email address already exists in an associated website"));
                                        $resultRedirect = $redirect->create();
                                        $resultRedirect->setPath('customer/account');

                                        return $resultRedirect;
                                    } else {
                                        $logger->info("same email with kakao ");
                                        $customer = $this->customerRepository->getById($customerSession->getId());
                                        $logger->info("Kakao email : " . $email);
                                        $logger->info("Kakao session email : " . $sessionEmail);
                ...
                if ($existCustomer) {
                    ...
                    return $this->doLoginAndRedirectExisting($existCustomer, $kakaoCustomerId, $tokenData, $userInfo);
                } else {
    ...
    protected function doLoginAndRedirectExisting($customer, $kakaoCustomerId, $tokenData, array $userInfo = [])
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/customer_oauth.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info("==========do Login And Redirect Existing=========");
        if (! $customer instanceof \Magento\Customer\Api\Data\CustomerInterface) {
            $logger->info('doLoginAndRedirectExisting called with invalid customer.');
            return;
        }
        try {
            $customerToSave = $this->customerRepository->getById($customer->getId());
        } catch (\Throwable $e) {
            $logger->info('Unable to load customer for update: ' . $e->getMessage());
            $customerToSave = null;
        }
        if (!empty($userInfo) && $customerToSave) {
            try {
                $logger->info('Updating customer data from Kakao info for customer id: ' . $customer->getId());
                $customerToSave = $this->customerRepository->getById($customer->getId());

                $logger->info("Inside user info not empty");
                if (isset($userInfo['kakao_account']['name'])) {
                    $fullName = trim($userInfo['kakao_account']['name']);
                    $nameParts = explode(' ', $fullName, 2);
                    $customerToSave->setFirstname($nameParts[0]);
                    $customerToSave->setLastname(isset($nameParts[1]) ? $nameParts[1] : '');
                    $logger->info('Set firstname/lastname from Kakao: ' . $customerToSave->getFirstname() . ' / ' . $customerToSave->getLastname());
                }
                ...
                if (isset($userInfo['kakao_account']['birthyear']) && isset($userInfo['kakao_account']['birthday'])) {
                    ...
                    $birthday = $userInfo['kakao_account']['birthday'];
                    $formattedBirthday = substr($birthday, 0, 2) . '/' . substr($birthday, 2, 2);
                    $dob = $birthYear . '/' . $formattedBirthday;
                    $customerToSave->setDob($dob);
                    $logger->info('Set DOB from Kakao: ' . $dob);
                }
                // Update Gender
                if (isset($userInfo['kakao_account']['gender'])) {
                    $gender = ' ';
                    ...
                    $logger->info('Set gender from Kakao: ' . $gender);
                }
                // Update Phone number
                //$logger->info("phone number: ". $userInfo['kakao_account']['phone_number']);
                if (!empty($userInfo['kakao_account']['phone_number'])) {
                    $phoneNumber = trim($userInfo['kakao_account']['phone_number']);
                    $parts = explode(' ', $phoneNumber, 2);
                    if (isset($parts[1])) {
                        ...
                        $cellPhone = trim($mobileNumber);
                    } else {
                        $cellPhone = trim($phoneNumber);
                    }
                    $customerToSave->setCustomAttribute('cell_phone', $cellPhone);
                    $logger->info('Set cell_phone from Kakao: ' . $cellPhone);
                }
                $customerToSave->setCustomAttribute('is_kakao_login', 1);
                $this->customerRepository->save($customerToSave);
                $logger->info('Customer updated successfully from Kakao info.');
            } catch (\Throwable $e) {
                $logger->info('Error updating customer from Kakao info: ' . $e->getMessage());
            }
        }
        $this->customerAuthorize->saveTokenData($customer->getId(), $kakaoCustomerId, $tokenData);
        $customerModel = $this->customerAuthorize->convertToCustomerModel($customer);
