...
/**
 * Class Cancel
 * @package OnitsukaTiger\Sales\Controller\Customer
 */
class Cancel extends \Magento\Framework\App\Action\Action implements HttpPostActionInterface, HttpGetActionInterface
{
    ...
    public function execute()
    {
        if (!$this->customerSession->isLoggedIn()) {
            return $this->resultRedirectFactory->create()->setPath('customer/account/history');
        }
        $order = $this->orderRepository->get($this->getRequest()->getParam('oar_order_id'));
        $response = [
            'success' => 1
        ];
        if($order->getCustomerId() == $this->customerSession->getCustomer()->getEntityId()) {
            ...
            $order = $this->orderFactory->create()->load($this->getRequest()->getParam('oar_order_id'));
