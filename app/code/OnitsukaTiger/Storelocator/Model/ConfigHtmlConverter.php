...
namespace OnitsukaTiger\Storelocator\Model;
...
class ConfigHtmlConverter extends \Amasty\Storelocator\Model\ConfigHtmlConverter
{
    ...
    public function __construct(
        ...
        parent::__construct($configProvider, $escaper, $filterProvider, $logger, $countryFactory, $regionFactory, $urlBuilder, $baseImageLocation, $dataHelper, $variableRenderers, $storeManager);
    }
    ...
    private function replaceLocationValues($template)
    {
        ...
        if (!isset($locationData['attributes']) && empty($locationData['attributes'])) {
            $location = $this->location->load($locationData['id']);
            $locationData['attributes'] =  $location->getAttributes();
        }
    ...
    private function convertAttributeData($attributeData)
    {
        ...
        if (isset($attributeData['option_title']) && is_array($attributeData['option_title'])) {
            foreach ($attributeData['option_title'] as $option) {
                $html .= $this->escaper->escapeHtml($option);
            }
            return $html;
        } else {
