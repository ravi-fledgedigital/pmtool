...
class SyncMissingOrders extends \OnitsukaTiger\Command\Console\Command
{
    const OPTION_DAYS_BEFORE = 'days';

    const CHARGE_PER_PAGE = 100;

    const OMISE_NETSUITE_GENERAL_MISSING_ORDERS_IDS = 'omise_netsuite/general/missing_order_ids';
    ...
    protected function configure()
    {
        ...
        $this->setName('omise:sync:missing_orders');
        $this->setDescription('Omise Sync Missing Orders Command');
        $this->setDefinition($options);
        parent::configure();
    }

    /**
     * Get missing orders array.
     *
     * @param $storeId
     * @return array|string[]
     */
    private function getMissingOrdersArray()
    {
        $missingOrders = $this->scopeConfig->getValue(
            self::OMISE_NETSUITE_GENERAL_MISSING_ORDERS_IDS,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );

        $missingOrdersArray = [];
        if (!empty($missingOrders)) {
            $missingOrdersArray = explode(',', $missingOrders);
        }
        return $missingOrdersArray;
    }
...
//        define('OMISE_API_VERSION', '2019-05-29');
        ...
        $missed_array = $this->getMissingOrdersArray();
        ...
        foreach ($transactions as $transaction) {
            ...
            if (!in_array($data['order_id'], $missed_array)) {
                $this->logger->alert('Static Order Sync To Omise : ' . $data['order_id']);
                $this->logger->alert('Missed Order IDS : ' . json_encode($missed_array));
                continue;
            }
            $result[] = $data;
        }
        $this->logger->alert('Result: ' . json_encode($result));
        if (count($result) == 0) {
            return self::SUCCESS;
        }
...
//                $data['commission_amount'] = (($charge['fee'] + $charge['fee_vat']) / 100);
//                $data['amount'] = ($transaction['amount'] / 100);
//            } else {
//                $data['transaction'] = $transaction['object'];
//                $data['commission_amount'] = 0;
//                $data['amount'] = ($transaction['amount'] / 100) - (($charge['fee'] + $charge['fee_vat']) / 100);
//            }
...
//        }
        ...
        foreach ($result as $row) {
            $this->logger->alert('Reciept Details : ' . json_encode($reciept));
            $result = [
                $row['transaction'],
                $this->formatDateForNetSuite($row['date']),
                $row['order_id'],
                $row['amount'],
                ...
                $row['type'],
                $reciept['number'],
                $this->formatDateForNetSuite($reciept['created_at']),
                $row['status']
            ];
    ...
    {
        $filename = sprintf('omise_%s_recover.csv', $csvDate->format('Ymd'));
        $data = [
            'filename' => $filename,
