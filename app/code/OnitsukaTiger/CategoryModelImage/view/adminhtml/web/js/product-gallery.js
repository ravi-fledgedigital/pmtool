...
define([
    ...
    $.widget('mage.productGallery', {
        options: {
            ...
            imgPostionSelector: '[data-role=img-positions]',
        ...
        _updateImageTitle: function (event, data) {
            var imageData = data.imageData,
                ...
                $position = $imgContainer.find(this.options.imgPostionSelector),
                value,
                positionValue = 'Position : ' + data.imageData.positions;
    ...
    $.widget('mage.productGallery', $.mage.productGallery, {
        ...
        _initDialog: function () {
            ...
            $dialog.on('change', '[data-role="image-description"]', function (e) {
                ...
                if(target.hasClass('admin__control-textarea')){
                    imageData.label = desc;
                    imageData['label_default'] = desc;
                }else{
                    imageData[target.attr('data-image')] = desc;
                    if(target.hasClass('position-image')) {
                        target.next().hide();
                        ...
                        if(isNaN(desc)){
                            ...
                            return false;
                        }else if (parseInt(desc) < 0){
                            target.next().next().show();
                            target.addClass('error');
                            return false;
                        }else{
                            $('input#positions-image-field').each(function(){
                                if(parseInt(desc) == $(this).val() && target.attr('name') != $(this).attr('name')) {
                    ...
                    if(target.hasClass('admin__actions-switch-checkbox')){
                        desc = target.is(':checked') ? 1 : 0;
                        imageData[target.attr('data-image')] = desc;
                        this.element.trigger('updateVisibility', {
