...
class TransferPayment extends AbstractMethod
{
    public const XML_PATH_IS_ENABLED = 'payment/portonetransfer/active';

    protected $_code = 'portonetransfer';
    ...
    {
        if ($amount <= 0) {
            $writer1 = new \Zend_Log_Writer_Stream(BP . '/var/log/issueWithPortOne.log');
            $logger1 = new \Zend_Log();
            $logger1->addWriter($writer1);
            $logger1->info('Amount     '. $amount);
            $logger1->info('*************************************************************');
        ...
        try {
            ...
            $payment->addTransaction(
                ...
                null,
                false,
                'Online refund via PortOne Bank Transfer'
            );
