...
namespace OnitsukaTiger\ProductAlert\Rewrite\Magento\ProductAlert\Model;
...
use OnitsukaTiger\ProductAlert\Block\Email\StockPreorder;
use OnitsukaTiger\ProductAlert\Block\Email\StockNew;
...
class Email extends \Magento\ProductAlert\Model\Email
{
    ...
    const XML_PATH_EMAIL_STOCK_PREORDER_TEMPLATE = 'catalog/productalert/email_stock_preorder_template';
    ...
    protected function _getStockBlock()
    {
        if ($this->_stockBlock === null) {
            $this->_stockBlock = $this->_productAlertData->createBlock(StockNew::class);
        }
        return $this->_stockBlock;
    }

    protected function _getStockPreorderBlock()
    {
        if ($this->_stockPreorderBlock === null) {
            $this->_stockPreorderBlock = $this->_productAlertData->createBlock(StockPreorder::class);
        }
        return $this->_stockPreorderBlock;
    }
    ...
    public function send()
    {
        ...
        if (!in_array($this->_type, ['price', 'stock', 'stock_preorder']) || count($products) === 0 || !$templateConfigPath) {
            return false;
        }
    ...
    private function getBlock(): AbstractEmail
    {
        ...
        } else if ($this->_type === 'stock') {
            return $this->_getStockBlock();
        } else if ($this->_type === 'stock_preorder') {
            return $this->_getStockPreorderBlock();
        }
    }
    ...
    private function getTemplateConfigPath(): string
    {
        ...
        switch ($this->_type) {
            case 'price':
                $template = self::XML_PATH_EMAIL_PRICE_TEMPLATE;
                break;
            case 'stock':
                $template = self::XML_PATH_EMAIL_STOCK_TEMPLATE;
                break;
            case 'stock_preorder':
                $template = self::XML_PATH_EMAIL_STOCK_PREORDER_TEMPLATE;
                break;
        };
