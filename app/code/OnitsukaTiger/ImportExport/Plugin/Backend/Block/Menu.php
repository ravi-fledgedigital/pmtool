...
namespace OnitsukaTiger\ImportExport\Plugin\Backend\Block;
...
class Menu
{
    ...
    {
        if ($level !== 0 && $menu->get('OnitsukaTiger_ImportExport::korea')) {
            $level = 0;
            $limit = self::MAX_ITEMS;
            if (is_array($colBrakes)) {
                foreach ($colBrakes as $key => $colBrake) {
                    if (isset($colBrake['colbrake']) && $colBrake['colbrake']
                    ) {
