...
class Collection extends \Magento\Framework\View\Element\UiComponent\DataProvider\SearchResult
{
    ...
    protected function _initSelect()
    {
        ...
        $this->getSelect()->from(['main_table' => new \Zend_Db_Expr("(
            SELECT *, 
                CASE
                    ...
                    AND sku like (SELECT concat((select sku from catalog_product_entity where entity_id = base_wli.product_id), '_', color, '_%'))
            ...
            FROM
                (SELECT
                    ...
                    `inner_base`.`new_product_id` AS entity_id,
                    `inner_base`.`color`,
                ...
                FROM   
                        (SELECT
                            *,
                            ...
                            option_id = json_unquote(json_extract(base_wlio.wlia_value, concat('$.','".$jc."',(SELECT attribute_id FROM eav_attribute WHERE entity_type_id=4 AND attribute_code='color'),'".$jc."')))
                ...
                GROUP BY
                inner_base.new_product_id,color
            ) AS base_wli
        ...
        $this->getSelect()->columns([
            "sku" => new \Zend_Db_Expr("`product`.`sku`"),
            "fku_id" => new \Zend_Db_Expr("`main_table`.`entity_id`"),
            "display_id" => new \Zend_Db_Expr("`main_table`.`entity_id`"),
            "display_type_id" => new \Zend_Db_Expr("`product`.`type_id`")
        ]);

        $this->getSelect()->distinct(true);
        $this->getSelect()->group(new \Zend_Db_Expr('main_table.entity_id'));
    ...
    public function addFilterMapColumn()
    {
        $columnsToMap = [
            ...
            [
                "alias"     => "sku",
                "fields"    => new \Zend_Db_Expr('product.sku')
            ],
            [
                "alias"     => "fku_id",
            ...
            [
                "alias"     => "total_favorites",
                "fields"    => new \Zend_Db_Expr('main_table.total_favorites')
            ],
            [
                "alias"     => "display_type_id",
                "fields"    => new \Zend_Db_Expr('product.type_id')
            ]
    ...
    protected function _renderFiltersBefore() {
        ...
        if (!empty($filters['search'])) {
            ...
            $this->getSelect()->where(new \Zend_Db_Expr("
                ...
                OR concat(concat(`product`.`sku`,'_'), `main_table`.`color`) LIKE '%" . $filters['search'] . "%' 
