...
class RefundBuilder implements \WeltPixel\GA4\Api\ServerSide\Events\RefundBuilderInterface
{
    ...
    public function getRefundEvent($creditmemo)
    {
        ...
        if ($currencyAndAmountScope == \WeltPixel\GA4\Model\Config\Source\CurrencyAndAmount::BASE_SCOPE) {
            $currencyCode = $creditmemo->getBaseCurrencyCode();
        } else {
        ...
        if ($order->getData('ga_gclid')) {
            $gaGclid = $order->getData('ga_gclid');
            $refundEvent->setGclid($gaGclid);
        }
        ...
        $refundEvent->setStoreId($order->getStoreId());
        ...
        $refundEvent->setValue($this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($creditmemo->getGrandTotal(), 2, '.', ''))));
        $refundEvent->setShipping($this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($creditmemo->getShippingAmount(), 2, '.', ''))));
        $refundEvent->setTax($this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($creditmemo->getTaxAmount(), 2, '.', ''))));
        ...
        foreach ($items as $creditmemoItem) {
            ...
            $productItemOptions['price'] = $this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($creditmemoItem->getPrice(), 2, '.', '')));
