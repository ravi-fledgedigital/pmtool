...
class RemoveFromCartBuilder implements \WeltPixel\GA4\Api\ServerSide\Events\RemoveFromCartBuilderInterface
{
    ...
    public function getRemoveFromCartEvent($product, $quantity, $quoteItem)
    {
        ...
        if ($this->ga4Helper->excludeTaxFromProductPrices()) {
            $productPrice = $this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($this->ga4Helper->convertPriceToCurrentCurrency($quoteItem->getPrice()), 2, '.', '')));
        } else {
            $productPrice = $this->ga4Helper->parsePriceBasedOnOptions(floatval(number_format($this->ga4Helper->convertPriceToCurrentCurrency($quoteItem->getPriceInclTax()), 2, '.', '')));
        }
        ...
        if ($gclid) {
            $removFromCartEvent->setGclid($gclid);
        }
