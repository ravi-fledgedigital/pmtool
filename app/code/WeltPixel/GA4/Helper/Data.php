...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @var array
     */
    protected $_pinterestPixelOptions;
     ...
     * @var array
     */
    protected $_bingPixelOptions;

    /**
     * @var array
     */
    protected $_xPixelOptions;

    /**
     * @var array
     */
    protected $_klaviyoPixelOptions;
    ...
    /**
     * @var \WeltPixel\GA4\Model\IpAddressResolver
     */
     ...
     * @param \Magento\Catalog\Helper\Product $productUrlHelper
     * @param \WeltPixel\GA4\Model\IpAddressResolver $ipAddressResolver
    ...
    public function __construct(
        ...
        \Magento\Catalog\Helper\Product $productUrlHelper,
        \WeltPixel\GA4\Model\IpAddressResolver $ipAddressResolver,
        ...
        $this->_bingPixelOptions = $this->scopeConfig->getValue('weltpixel_ga4_bing_pixel', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $this->_xPixelOptions = $this->scopeConfig->getValue('weltpixel_ga4_x_pixel', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $this->_klaviyoPixelOptions = $this->scopeConfig->getValue('weltpixel_ga4_klaviyo_pixel', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $this->_pinterestPixelOptions = $this->scopeConfig->getValue('weltpixel_ga4_pinterest_pixel', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
    ...
    */
     ...
     * @return boolean
     */
    public function excludeTaxFromProductPrices()
    {
        return $this->_gtmOptions['general']['exclude_tax_from_product_prices'];
    }
     ...
     * @return boolean
     */
    public function includeTaxInShippingValue()
    {
        return $this->_gtmOptions['general']['include_tax_in_shipping_value'];
    }
     ...
     * @return bool
     */
    public function isPreconnectUrlEnabled()
    {
        return (bool)$this->_gtmOptions['general']['preconnect_url'];
    }
    ...
    public function getGtmCodeSnippetForHead()
    {
        ...
        if ($scriptAttributes) {
            $gtmCodeSnippet = str_replace('<script', '<script ' . $scriptAttributes, $gtmCodeSnippet);
        }
    ...
    {
        $serializedScriptAttributes = $this->_gtmOptions['general']['script_attributes'] ?? '';
        $scriptAttributesString = ' ';
        if ($serializedScriptAttributes) {
            $scriptAttributes = $this->serializer->unserialize($serializedScriptAttributes);
            foreach ($scriptAttributes as $attribute) {
                $scriptAttributesString .= $attribute['script_attribute'] . ' ';
            }
        }

        return $scriptAttributesString;
    }
    ...
    public function addToCartPushData($qty, $product, $buyRequest = [], $checkForCustomOptions = false)
    {
        ...
        if ($this->checkoutSession->getGA4LastProductPrice()) {
            $productData['price'] = $this->parsePriceBasedOnOptions(floatval(number_format($this->convertPriceToCurrentCurrency($this->checkoutSession->getGA4LastProductPrice()), 2, '.', '')));
            $this->checkoutSession->setGA4LastProductPrice(null);
        } else {
            $productFinalPriceWithTaxCalculations = $this->getProductPriceWithTaxCalculations($product, $product->getPriceInfo()->getPrice('final_price')->getValue());
            $productData['price'] = $this->parsePriceBasedOnOptions(floatval(number_format($productFinalPriceWithTaxCalculations, 2, '.', '')));
        }
        ...
        $result['ecommerce']['currency'] =  $this->getCurrencyCodeBasedOnOptions();
    ...
    public function convertPriceToBaseCurrency($price)
    {
        ...
        $convertedPrice = $this->priceCurrency->convert($price);
        if ($convertedPrice <= 0) {
            return $price;
        }
     ...
     * @return float
     */
    public function parsePriceBasedOnOptions($price)
    {
        $currencyAndAmountScope = $this->getCurrencyAndAmountScope();
        if ($currencyAndAmountScope == \WeltPixel\GA4\Model\Config\Source\CurrencyAndAmount::BASE_SCOPE) {
            $price = $this->convertPriceToBaseCurrency($price);
     ...
     * @return float
     */
    public function getProductPriceWithTaxCalculations($product, $price)
    {
        if ($this->excludeTaxFromProductPrices()) {
            return $price;
        }
    ...
    public function getCurrencyCodeBasedOnOptions()
    {
        $currencyAndAmountScope = $this->getCurrencyAndAmountScope();
        if ($currencyAndAmountScope == \WeltPixel\GA4\Model\Config\Source\CurrencyAndAmount::BASE_SCOPE) {
        ...
        switch ($idOption) {
            ...
            default:
                ...
                if ($product instanceof \Magento\Sales\Model\Order\Item) {
                    $gtmProductId = $product->getProductId();
                }
                break;
        }
     ...
     * @return string
     */
    public function getCurrencyAndAmountScope()
    {
        return $this->_gtmOptions['general']['currency_and_amount_scope'];
    }
    ...
    public function removeFromCartPushData($qty, $product, $quoteItem)
    {
        ...
        $productData['item_id'] = $productId;
        if ($this->excludeTaxFromProductPrices()) {
            $productData['price'] = $this->parsePriceBasedOnOptions(floatval(number_format($this->convertPriceToCurrentCurrency($quoteItem->getPrice()), 2, '.', '')));
        } else {
            $productData['price'] = $this->parsePriceBasedOnOptions(floatval(number_format($this->convertPriceToCurrentCurrency($quoteItem->getPriceInclTax()), 2, '.', '')));
        }
        if ($this->isBrandEnabled()) {
            $productData['item_brand'] = $this->getGtmBrand($product);
        }
    ...
    public function addCheckoutStepPushData($step, $checkoutOption, $order = null)
    {
        ...
        $checkoutStepResult['ecommerce']['currency'] = $this->getCurrencyCodeBasedOnOptions();
        if ($order) {
            $orderValue = $this->orderTotalCalculator->calculateOrderTotal($order, $this);
            $checkoutStepResult['ecommerce']['value'] = $this->parsePriceBasedOnOptions(floatval(number_format($orderValue, 2, '.', '')));
        } else {
            $checkoutStepResult['ecommerce']['value'] = $this->parsePriceBasedOnOptions(floatval(number_format($this->getCartTotal(), 2, '.', '')));
        }
    ...
    public function getCartTotal()
    {
        ...
        if (!$grandTotal) {
            ...
            if ($lastOrderId) {
                $order = $this->orderRepository->get($lastOrderId);
                $grandTotal = $this->orderTotalCalculator->calculateOrderTotal($order, $this);
            }
        }
    ...
    {
        return $this->_gtmOptions['adwords_conversion_tracking']['enable_enhanced_conversion_filter'] ?? false;
    }
    ...
    {
        $attributes = $this->_gtmOptions['adwords_conversion_tracking']['enhanced_conversion_attributes'] ?? '';
     ...
     * @param string $list
     * @param string $listId
     * @param string $htmlResult
     * @return string
     */
    public function addProductClick($product, $index = 0, $list = '', $listId = '', $htmlResult = true)
    {
        $html = $this->getProductClickHtml($product, $index, $list, $listId, $htmlResult);

        if ($htmlResult && !empty($html)) {
        ...
        if (!$htmlResult && !empty($html)) {
            $htmlArrayObject = new \Magento\Framework\DataObject(['htmlArray' => $html]);
            $this->_eventManager->dispatch('weltpixel_ga4_afterproductclick', [
                'html' => '',
                'htmlArray' => $htmlArrayObject,
                'product' => $product,
                'index' => $index,
                'list' => $list
            ]);

            $htmlArray = $htmlArrayObject->getData('htmlArray');
            return $htmlArray;
        }
     ...
     * @param string $list
     * @param string $listId
     * @param bool $htmlResult
     * @return string | array
     */
    public function getProductClickHtml($product, $index = 0, $list = '', $listId = '', $htmlResult = true)
    {
        ...
        if ($productClickBlock) {
            ...
            if (!$htmlResult) {
                ...
                $serverSideGA4Data = $productClickBlock->getServerSideGA4Data();
                $frontendGA4Data = $productClickBlock->getFrontendGA4Data();

                return [
                    'serverSideGA4Data' => $serverSideGA4Data,
                    'frontendGA4Data' => $frontendGA4Data
                ];
            }
    ...
    */
    ...
    public function getCustomerOrderCount($customerId, $customerEmail)
    {
        ...
        $conditions = ['customer_email = :customer_email'];
        ...
        $select = $connection->select()
            ->from($tableName, ['total_orders' => new \Zend_Db_Expr('COUNT(*)')])
    ...
    {
        $remoteIpAddress = $this->ipAddressResolver->getIpAddress();
    ...
    {
        return \WeltPixel\GA4\Model\Config\Source\AddonOrderTotalCalculation::CALCULATE_DEFAULT;
    }
