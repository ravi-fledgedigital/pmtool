...
class License extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $matches = [];
        preg_match('/(.local|.localhost|.localdev|.dev|.development|.test|.staging|.stage|.ddev.site|.cloudwaysapps.com|magentosite.cloud|magento.cloud|nxcli.net|ddev.site|nxcli.io|magento2.docker)$/', $domain, $matches);
        if (isset($matches[1])) {
            if (($matches[1] == 'magentosite.cloud') || ($matches[1] == 'magento.cloud') || ($matches[1] == 'magento2.docker') || ($matches[1] == 'ddev.site') || ($matches[1] == 'nxcli.net') || ($matches[1] == 'nxcli.io')) {
                return true;
            }
        ...
        if (count($domainParts) > count($licenseParts)) {
            $domainToCompare = implode('.', array_slice($domainParts, -(count($licenseParts))));
            return $domainToCompare === $licenseDomain;
        }
        ...
        if ($forced) {
           if ($moduleLicenseDetails['module'] != $mdN) {
            ...
            if (count($this->_wpBundleList)) {
                $mdN = $this->getBundleNameForModule($mdN, $this->_wpBundleList);
            }
     ...
     * @return string
     */
    public function getBundleNameForModule($mdN, $wpBundleList) {
        foreach ($wpBundleList as $bundleName => $modules) {
            if (in_array($mdN, $modules)) {
                return $bundleName;
            }
