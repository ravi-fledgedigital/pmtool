        ...
        return 'INSERT INTO crm_analysis_data (
         ...
         FROM ((
            ...
            SELECT customer_id AS member_id,
                ...
                COUNT(CASE WHEN STATUS IN("closed", "canceled", "partial_refund","rejected") THEN 1 ELSE NULL END) AS total_return_times,
                SUM(CASE WHEN STATUS IN(' . $orderStatuses . ') THEN base_grand_total ELSE 0 END) AS total_purchase_amounts,
                SUM(CASE WHEN STATUS IN("closed", "canceled", "partial_refund","rejected") THEN CASE WHEN STATUS = "partial_refund" THEN base_total_refunded ELSE base_grand_total END ELSE 0 END) AS total_return_amounts
