    ...
    function(
        ...
        return Component.extend({
            ...
            selectPaymentMethodType: function() {
                ...
                $('body').addClass('active-payment-method');
                return true;
            },
            /**
            ...
            showErrorMessage: function(response) {
                if (!!response['responseJSON'].parameters) {
                    ...
                    text(($t(response['responseJSON'].message)).replace('%1',
                        response['responseJSON'].parameters[0])).
                    slideDown();
                } else {
                    $('#messages-' + selectedAlternativePaymentMethodType()).
                    text($t(response['responseJSON'].message)).
                    slideDown();
                }
            ...
            activeMethod: function(paymentMethod) {
                if(!$('.checkout-payment-method .opc-payment .payment-method._active input[name="payment[method]"][id*="'+paymentMethod.methodIdentifier+'"]').is(":checked")) {
                    $('.checkout-payment-method .opc-payment .payment-method._active input[name="payment[method]"][id*="'+paymentMethod.methodIdentifier+'"]').trigger('click');
                    $('.checkout-payment-method .opc-payment .payment-method._active input[name="payment[method]"][id*="'+paymentMethod.methodIdentifier+'"]').prop("checked",true);
                }
            },
            getRatePayDeviceIdentToken: function() {
                return window.checkoutConfig.payment.adyenHpp.deviceIdentToken;
