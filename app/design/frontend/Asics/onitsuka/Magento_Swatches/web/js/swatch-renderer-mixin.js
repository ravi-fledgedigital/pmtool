        ...
        $.widget('mage.SwatchRenderer', widget, {
                ...
                $.each(this.options.jsonConfig.attributes, function () {
                    ...
                    if ($widget.options.enableControlLabel) {
                        if ($('body').hasClass('catalog-category-view')) {
                            if((item.code == 'size' || item.code == 'qa_size') &&
                                !$('body').hasClass('web_kr_ko') && !$('body').hasClass('web_id_id') &&
                                !$('body').hasClass('web_id_en') && !$('body').hasClass('web_vn_vi') &&
                                !$('body').hasClass('web_vn_en') && $('.product.gender.value').length) {
                            ...
                            if((item.code == 'size' || item.code == 'qa_size') &&
                                ...
                                !$('body').hasClass('web_id_en') && !$('body').hasClass('web_vn_vi') &&
                                !$('body').hasClass('web_vn_en') && $('.product.gender.value').length) {
                                label += '<span class="text-size-custom">사이즈를 선택</span>';
                                label += '<span class="size-guide-product '
                                    + $('<i></i>').text(item.label).html().toLowerCase() + '">'
                                    + $t('Size guide') +
                                    '</span>';
                                if($('.product.hidden.gender').length) {
                                    label += '<span id="' + controlLabelId + '" class="'
                                        + $('<i></i>').text(item.label).html().toLowerCase() + ' '
                                        + classes.attributeLabelClass + '">'
                                        + 'US ' + $t(item.label) + ' ('
                                        + $('.product.hidden.gender').text().trim() + ')'
                                        + '</span>';
                                } else {
                                    label += '<span id="' + controlLabelId + '" class="'
                                        + $('<i></i>').text(item.label).html().toLowerCase() + ' '
                        ...
                        if (jsonResponse) {
                            ...
                            if ($('body').hasClass('web_kr_ko')) {
                                if (responseData.coming_soon) {
                                    $('.coming-soon').show();
                                    $('.coming-soon .coming-soon-label').text(responseData.coming_soon_label);
                                    $("#button-alert-custom").text("출시 안내 받기");
                                } else {
                                    $('.coming-soon').hide();
                                    $('.coming-soon .coming-soon-label').text('');
                                }
                            } else {
                                if(responseData.coming_soon) {
                            ...
                            if ($('body').hasClass('web_kr_ko')) {
                                if(responseData.all_product_restock === 0 && responseData.all_product_oos === 1) {
                                ...
                                } else if(responseData.all_product_restock === 1 && responseData.all_product_oos === 1) {
                                    $('#button-alert-custom').show();
                                    $('.box-quick-purchase').hide();
                                    $('.preorder_note').empty();
                                    $('.product-options-bottom').find('.box-tocart').find('#product-addtocart-button').attr('style', 'display: none !important');
                                    $('#button-oos-custom').attr('style', 'display: none !important');
                                } else if(responseData.coming_soon){
                                    $('.product-options-bottom').find('.box-tocart').find('#product-addtocart-button').attr('style', 'display: none !important');
                                    $('.box-quick-purchase, #product-quick-purchase-button').attr('style', 'display: none !important');
                                }
                            } else {
                                if (responseData.all_product_oos === 1 && responseData.coming_soon) {
                                    $('#button-oos-custom').attr('style', 'display: none !important');
                                    $('.product-options-bottom').find('.box-tocart').find('#product-addtocart-button').attr('style', 'display: none !important');
                                    $('.box-quick-purchase, #product-quick-purchase-button').attr('style', 'display: none !important');
                                }
            ...
            _OnClick: function ($this, $widget) {
                ...
                setTimeout(function () {
                    ...
                    if ($this.hasClass('pre-order-item-disabled')) {
                        $('.box-quick-purchase, #product-quick-purchase-button').attr('style', 'display: none !important');
                        $('.content-wrap.item-detail .block-btn-sp .btn-cart').prop("disabled", true);
                        $('.product-options-bottom').find('.box-tocart').find('#product-addtocart-button').addClass('disabled');
                    }
                }, 10);
            ...
            _Rebuild: function () {
                ...
                if (selectedOptionValue.hasClass('pre-order-item') || selectedOptionValue.hasClass('pre-order-item-disabled')) {
                ...
                $('.preorder_note').empty();
                if (buttonTitle.toLowerCase() === 'pre-order') {
                    $('.box-quick-purchase, #product-quick-purchase-button').attr('style', 'display: none !important');
                } else {
