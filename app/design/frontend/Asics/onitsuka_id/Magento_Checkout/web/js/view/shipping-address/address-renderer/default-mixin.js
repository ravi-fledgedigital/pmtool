...
define([
    ...
    var mixin = {
        ...
        isDefaultShipping: function () {
            if (!this.isSelected()) {
                var shippingAddress = quote.shippingAddress(),
                    addressKey = checkoutData.getSelectedShippingAddress(),
                    defaultShippingAddress = window.customerData.default_shipping;
                if (shippingAddress) {
                    if (addressKey && addressKey.indexOf('customer-address') == 0) {
                        if (addressKey == this.address().getKey()) {
                            return true;
                        }
                    } else if (defaultShippingAddress == this.address().customerAddressId) {
                        return true;
                    }
                }
            }
        ...
        _bindAddress: function () {
            ...
            $('#opc-new-shipping-address').find('[name="postcode"]').val(self.address().postcode).trigger('change');
            if (self.address().customAttributes.length > 0) {
                $('#opc-new-shipping-address').find('[name="custom_attributes[district]"]').val(self.address().customAttributes[0].value).trigger('change');
            } else {
                $('#opc-new-shipping-address').find('[name="custom_attributes[district]"]').val('');
            }
        },
        /**
         *
        ...
        _clearFormData: function (){
            ...
            $('.form-shipping-address').find('[name="custom_attributes[district]"]').val('').trigger('change');
