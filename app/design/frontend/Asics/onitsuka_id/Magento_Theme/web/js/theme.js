...
define([
    ...
    $t("The shipping method can't be set for an empty cart. Add an item to cart and try again.");
    $t('Success, Credit card transaction is successful');
    $t('Success, transaction is found');
    $t('The shipping information was unable to be saved. Error: "Invalid address id "');
    $t('Current customer does not have an active cart.');
    ...
    $('.banner-slide-custom .pagebuilder-column-group').addClass('swiper-wrapper');
    ...
    if(!$('.header-alert .slick-list').length) {
    ...
    function footerBox() {
        if($(window).width() <768) {
            ...
            $('.footer-box-wrapper').insertAfter('.page-footer .footer.content');
        }
    }
    if(!$('body').hasClass('checkout-index-index')) {
        $(document).on('change',"[name='region_id']",function(){
            ...
            $(this).parents('form').find("[name='postcode']").val("").trigger('change');
            if($(this).parents('form').find("[name='country_id']").val() == 'MY') {
                if ($(this).find('option:selected').text() == "Wilayah Persekutuan Labuan") {
                    $(this).parents('form').find("[name='postcode']").prop('readonly', true);
                    $(this).parents('form').find("[name='postcode']").val("87000").trigger('change');
                }
            }
        });
    }
