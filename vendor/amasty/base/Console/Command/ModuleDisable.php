...
class ModuleDisable extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $filteredValues = $this->getDependencyProvider()
            ->getDependencies($modulesToCheck, $selectedFlag, false);

        $status = $this->getModuleStatus();
        $modulesToChange = $status->getModulesToChange(false, $filteredValues);
        if (!empty($modulesToChange)) {
            $status->setIsEnabled(false, $modulesToChange);
            $output->writeln('<info>The following modules have been disabled:</info>');
            foreach ($filteredValues as $module) {
    ...
    {
        $this->setName('amasty:module:disable')
            ->setDescription('Disable modules with additional Amasty filters')
            ->addArgument(self::ARG_MODULES, InputArgument::IS_ARRAY, 'Module names to disable')
            ->addOption(
                DependencyProvider::FLAG_AMASTY,
                null,
                InputOption::VALUE_NONE,
                'Disable all Amasty modules (except Base)'
            )
            ->addOption(
                DependencyProvider::FLAG_EXTERNAL,
                null,
                InputOption::VALUE_NONE,
                'Disable all third-party vendor modules'
            )
            ->addOption(
                DependencyProvider::FLAG_ALL,
                null,
                InputOption::VALUE_NONE,
                'Disable all third-party modules (except Magento/PayPal/Amasty Base)'
            )
            ->addOption(
                DependencyProvider::FLAG_DEP,
                null,
                InputOption::VALUE_NONE,
                'Disable provided modules and their dependencies'
            );
    }
