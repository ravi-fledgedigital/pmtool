...
class SearchBoxElement extends Template
{
    ...
    public function shouldRenderSearchBox(): bool
    {
        if (!$this->attributeConfig->isShowSearchBox()) {
            return false;
        }

        $limit = $this->sizeLimiterConfigProvider->getLinkLimit();
        return !$limit || $this->getFilter()->getItemsCount() > $limit;
    }
}
