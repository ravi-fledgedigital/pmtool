...
class AttributeFactor implements FactorInterface
{
    ...
    public function reindex(RankingFactorInterface $rankingFactor, array $productIds): void
    {
        ...
        $mappingValues = array_column($mapping, 'value');
        $mappingValues = array_filter($mappingValues, function($v) {
            return is_numeric($v);
        });
        $mappingValues = array_map('floatval', $mappingValues);

        $maxAbs = count($mappingValues) ? max(array_map('abs', $mappingValues)) : 0;
        

        $this->indexer->process($rankingFactor, $productIds, function () use ($stmt, $mapping, $maxAbs) {
            $mappingCount = count($mapping);
            while ($row = $stmt->fetch()) {
                $value = (float)$this->getValue($mapping, (string)$row['value']);
                $formattedValue = $this->formatValue($mapping, (string)$row['value']);

                $normalizedScore = $this->normalizeScore($value, $maxAbs, $mappingCount);

                $this->indexer->add((int)$row['entity_id'], $normalizedScore, $formattedValue);
            }
        });
    }

    private function normalizeScore(float $value, float $maxAbs, int $mappingCount): float
    {
        if ($mappingCount > 100) {
            if ($maxAbs == 0) {
                return 0.0;
