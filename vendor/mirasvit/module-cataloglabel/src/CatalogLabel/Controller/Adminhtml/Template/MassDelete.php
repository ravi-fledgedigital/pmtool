...
class MassDelete extends Template
{
    public function execute()
    {
        ...
        if (!is_array($ids) && $excluded !== 'false' && !is_array($excluded)) {
            $this->messageManager->addError((string)__('Please select template(s)'));
        } else {
            try {
                $templates = $this->repository->getCollection();

                if (is_array($ids)) {
                    $templates->addFieldToFilter(TemplateInterface::ID, ['in' => $ids]);
                } elseif (is_array($excluded)) {
                    $templates->addFieldToFilter(TemplateInterface::ID, ['nin' => $excluded]);
                } else {
                    $templates = $this->filter->getCollection($templates);
                }
