...
class GiftRegistryRegistrantsUpdater
{
     ...
     * @throws AlreadyExistsException|\Magento\Framework\Validator\ValidateException
     */
        ...
        foreach ($recipients as $recipient) {
            ...
            if (isset($recipient['id'])) {
                ...
                if ((int) $person->getEntityId() !== $giftRegistryId) {
                    $errors[] = __(
                        'The registrant with ID=%1 doesn\'t belong to the current gift registry.',
                        [$recipient['id']]
                    );
                    continue;
                }
                $recipient = $this->hydrateRecipientDetails($recipient, $person->getData());
            }
            $idField = $person->getIdFieldName();
            if (empty($recipient[$idField])) {
                unset($recipient[$idField]);
            }
    ...
    /**
     * Combines registrant details
     *
     * @param array $recipient
     * @param array|null $details
     * @return array
     */
    private function hydrateRecipientDetails(array $recipient, ?array $details): array
    {
        if (!$details) {
            return $recipient;
        }

        return array_merge($details, $recipient);
    }
}
