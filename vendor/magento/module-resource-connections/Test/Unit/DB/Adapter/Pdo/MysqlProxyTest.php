...
class MysqlProxyTest extends TestCase
{
     ...
     *
     * @return void
     * @dataProvider causalityCheckOnReadForSlaveDataProvider
     */
    public function testCausalityCheckOnReadForSlave(string $methodName, array $params): void
    {
        $config = $this->config;
        $config['slave']['synchronous_replication'] = true;
        $expectedMasterBuilderConfig = $config;
        unset($expectedMasterBuilderConfig['slave']);
        $expectedSlaveBuilderConfig = array_merge($expectedMasterBuilderConfig, $config['slave']);
        ...
        $this->slaveConnectionMock->expects($this->once())
            ->method('query')
            ->with('SET SESSION wsrep_sync_wait=1');
        ...
        call_user_func_array([$mysqlProxy, $methodName], $params);
    }

    /**
     * @return array
     */
    public function causalityCheckOnReadForSlaveDataProvider(): array
    {
        return [
            ['update', ['table', []]],
            ['insert', ['table', []]],
