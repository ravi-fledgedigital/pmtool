...
use Magento\Reminder\Model\ResourceModel\Rule\Collection as RuleCollection;
use Magento\Reminder\Model\Rule;
use Magento\Reminder\Model\Rule\Condition\Combine\RootFactory;
...
class RuleTest extends TestCase implements TestProvidesServiceInterface
{
    ...
    protected function setUp(): void
    {
        ...
        $this->timezoneMock = $this->getMockBuilder(TimezoneInterface::class)
            ->getMock();
        $this->rootFactoryMock = $this->getMockBuilder(RootFactory::class)
            ->disableOriginalConstructor()
            ->onlyMethods(['create'])
            ->getMock();
        ...
        $selectMock = $this->getMockBuilder(Select::class)
            ->disableOriginalConstructor()
            ->getMock();
        $this->ruleCollection = $this->getMockBuilder(AbstractDb::class)->disableOriginalConstructor()
            ->onlyMethods(['getResource', 'getSelect', '_fetchAll'])
            ->addMethods(['addDateFilter', 'addIsActiveFilter'])
            ->getMock();
        $this->ruleCollection->expects($this->any())->method('addDateFilter')->willReturnSelf();
        $this->ruleCollection->expects($this->any())->method('addIsActiveFilter')->willReturnSelf();
        $this->ruleCollection->expects($this->any())->method('getSelect')->willReturn($selectMock);
        $this->customerMock = $this->getMockBuilder(Customer::class)
            ->onlyMethods(['load', 'getId', 'getStore', 'getName'])
            ->addMethods(['getStoreId', 'getWebsiteId'])
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->serviceObjectsMap = [
            ExtensionAttributesFactory::class => $extensionAttributeFactoryMock,
            AttributeValueFactory::class => $attributeValueFactoryMock,
            Json::class => $this->getSerializerMock(),
            RuleCollection::class => $this->ruleCollection,
        ];
        $this->rule = new Rule(
            $this->contextMock,
            $this->registryMock,
    ...
    {
        return $this->serviceObjectsMap[$type] ?? null;
    }
     ...
     */
    private function getSerializerMock(): Json
    {
     ...
     * @return void
     */
    protected function setSavePreconditions(): void
    {
        ...
        $actionMock = $this->getMockBuilder(Collection::class)->disableOriginalConstructor()
            ->onlyMethods(['asArray'])
            ->addMethods(['setRule', 'setId', 'setPrefix'])
            ->getMock();
        ...
        $this->eventManagerMock
            ...
            ->willReturnCallback(function ($arg1) {
                if ($arg1 == 'model_save_before' || $arg1 == 'core_abstract_save_before') {
                    return null;
                }
            });
    }
     ...
     * @return void
     */
    public function testBeforeSaveNew(): void
    {
     ...
     * @return void
     */
    public function testBeforeSaveExisting(): void
    {
        $this->setSavePreconditions();
     ...
     * @return array
     */
    protected function setSendReminderEmailsPreconditions(
        string $label = 'label',
        ...
        $this->ruleCollection->expects($this->any())->method('_fetchAll')->willReturn([]);
     ...
     * @return void
     */
    public function testSendReminderEmailsWithException(): void
    {
     ...
     * @return void
     */
    public function testGetStoreDataWithNull(): void
    {
        $this->ruleResourceMock
            ->expects($this->once())
            ->method('getStoreTemplateData')
     ...
     * @return void
     */
    public function testDetachSalesRule(): void
    {
        $salesRuleId = 1;
        $this->ruleResourceMock
            ->expects($this->once())
