...
class UpdateTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $select->expects($this->exactly(2))
            ->method('where')
            ->willReturnCallback(function ($arg1, $arg2) use ($select) {
                if ($arg1 === 'rollback_id = ?' && $arg2 === $this->rollbackId) {
                    return $select;
                } elseif ($arg1 === 'id NOT IN (?)' && $arg2 === [$this->updateId]) {
                    return $select;
                }
            });
     ...
     * @return array
     */
    public static function rollbackAssignedToUpdatesDataProvider(): array
    {
        return [
            ['string', true],
            [false, false],
