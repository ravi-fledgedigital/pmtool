...
class UpdateRepositoryTest extends TestCase
{
    ...
    public function testShouldTriggerSyncOnEndTimeChange(): void
    {
        ...
        $this->resourceMock->expects($this->exactly(2))
            ->method('load')
            ->willReturnCallback(function ($arg1) use ($newUpdate, $getUpdate) {
                if ($arg1 == $newUpdate || $arg1 == $getUpdate) {
        ...
        $this->resourceMock->expects($this->exactly(2))
            ->method('save')
            ->willReturnCallback(function ($arg1) use ($rollback, $newUpdate) {
                if ($arg1 == $rollback || $arg1 == $newUpdate) {
                    return  $this->resourceMock;
                }
            });
