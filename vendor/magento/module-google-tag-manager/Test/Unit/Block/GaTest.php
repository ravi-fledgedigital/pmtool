...
class GaTest extends TestCase
{
    ...
    public function testGetOrdersData()
    {
        ...
        $finalResult = $this->ga->getOrdersData();
        $this->assertEquals("dataLayer.push(" . json_encode($result) . ");\n", $finalResult);
        $this->assertStringNotContainsString('"price":"85"', $finalResult);
        $this->assertStringNotContainsString('"quantity":"1"', $finalResult);
        $this->assertStringNotContainsString('"revenue":"120"', $finalResult);
        $this->assertStringContainsString('"price":85', $finalResult);
        $this->assertStringContainsString('"price":85.55', $finalResult);
        $this->assertStringContainsString('"quantity":1', $finalResult);
        $this->assertStringContainsString('"quantity":1.11', $finalResult);
        $this->assertStringContainsString('"revenue":120.55', $finalResult);
    }
    ...
    private function prepareOrderDataMocks(): array
    {
        ...
        $item2->expects($this->atLeastOnce())->method('getBasePrice')->willReturn(85.55);
        $item2->expects($this->atLeastOnce())->method('getQtyOrdered')->willReturn(1.11);

        $order = $this->createMock(Order::class);
        $order->expects($this->once())->method('getIncrementId')->willReturn('10002323');
        $order->expects($this->once())->method('getBaseGrandTotal')->willReturn(120.55);
        $order->expects($this->once())->method('getBaseTaxAmount')->willReturn(15.0);
        $order->expects($this->once())->method('getBaseShippingAmount')->willReturn(20.0);
