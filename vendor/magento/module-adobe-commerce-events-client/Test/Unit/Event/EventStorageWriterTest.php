...
class EventStorageWriterTest extends TestCase
{
    ...
    public function testEventHasAliases()
    {
        ...
        $this->validatorMock->expects(self::exactly(2))
            ->method('validate')
            ->willReturnCallback(function (Event $event, array $data) use ($eventMockTwo, $eventMockThree, $eventData) {
                static $count = 0;
                switch ($count++) {
                    case 0:
                        self::assertEquals($eventMockTwo, $event);
                        self::assertEquals($eventData, $data);
                        break;
                    case 1:
                        self::assertEquals($eventMockThree, $event);
                        self::assertEquals($eventData, $data);
                        break;
                };
                return true;
