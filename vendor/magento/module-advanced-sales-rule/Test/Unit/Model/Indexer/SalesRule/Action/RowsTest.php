...
class RowsTest extends TestCase
{
    ...
    public function testExecuteNoInstance()
    {
        ...
        $rule = $this->getMockBuilder($className)
            ->disableOriginalConstructor()
            ->onlyMethods(['load', 'getId', 'getConditions'])
    ...
    public function testExecuteWithInstance()
    {
        ...
        $this->filterResourceModel->expects($this->any())
            ->method('insertFilters')
            ->willReturnCallback(
                function ($arg1) use ($expectArray1, $expectArray2) {
                    if ($arg1 == $expectArray1 || $arg1 == $expectArray2) {
                        return $this->filterResourceModel;
                    }
                }
            );
        $this->model->execute($rows);
