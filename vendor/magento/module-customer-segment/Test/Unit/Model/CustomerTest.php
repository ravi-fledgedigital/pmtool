...
class CustomerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->_resource = $this->getMockBuilder(\Magento\CustomerSegment\Model\ResourceModel\Customer::class)
            ->onlyMethods(['getCustomerWebsiteSegments', 'getIdFieldName', 'addCustomerToWebsiteSegments'])
            ->setConstructorArgs(
                [
                    $contextMock,
     ...
     * @return void
     *
     * @dataProvider getDataForProcessEventForVisitor
     */
    public function testProcessEventForVisitor($customer, $quoteId)
    {
        $event = 'test_event';
        ...
        ->willReturnCallback(function ($arg1, $arg2) {
            if ($arg1 == 'apply_to' &&
                $arg2 == [Segment::APPLY_TO_VISITORS, Segment::APPLY_TO_VISITORS_AND_REGISTERED]) {
                return null;
            } elseif ($arg1 == 'conditions_serialized' &&
                $arg2 == ['nlike' => '%,"conditions":[%']) {
                return null;
            }
        });
        ...
        $this->visitorMock->setData('quote_id', $quoteId);
        ...
        $this->assertEquals($this->model, $this->model->processEvent($event, $customer, 1));
    }

    /**
     * @return array
     */
    public static function getDataForProcessEventForVisitor(): array
    {
        return [
            'no-customer-no-quote' => [null, null],
            'no-customer-quote' => [null, 1],
        ];
    }
    ...
    public static function dataProviderAddVisitorToWebsiteSegments()
    {
        return [
            ['', 1, [], [1 => []], []],
        ...
        $this->collection->method('addFieldToFilter')
            ->willReturnCallback(function ($arg1, $arg2) {
                if ($arg1 == 'apply_to' &&
                    $arg2 == [Segment::APPLY_TO_VISITORS, Segment::APPLY_TO_VISITORS_AND_REGISTERED]) {
                    return null;
                } elseif ($arg1 === 'conditions_serialized' && $arg2 === ['nlike' => '%,"conditions":[%']) {
                    return null;
                }
            });
        $connectionMock = $this->getMockForAbstractClass(AdapterInterface::class);
    ...
    public function testAddCustomerToWebsiteSegments(
        ...
        $this->httpContextMock->expects($this->once())
            ->method('setValue')
            ->with(Data::CONTEXT_SEGMENT, $value, [])
            ->willReturnSelf();
    ...
    public static function dataProviderRemoveVisitorFromWebsiteSegments()
    {
        return [
            ['', 1, [], [], []],
     ...
     * @return array
     */
    public static function dataProviderAddCustomerToWebsiteSegments(): array
    {
        return [
            [[], [], [], [1 => []], [1 => []]],
            [[], ['1'], ['1'], [1 => []], [1 => ['1']]],
