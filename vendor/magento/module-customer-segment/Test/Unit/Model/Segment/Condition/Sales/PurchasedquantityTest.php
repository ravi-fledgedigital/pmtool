...
class PurchasedquantityTest extends TestCase
{
    ...
    public function testGetConditionsSqlIsNotFilteredAndNumericWebsite($operator, $value, $attribute, $checkSql)
    {
        ...
        $select->expects($this->exactly(2))
            ->method('where')
            ->willReturnCallback(function ($arg1, $arg2) use ($select, $storeIds) {
                if ($arg1 == 'sales_order.customer_id IS NOT NULL') {
                    return $select;
                } elseif ($arg1== 'sales_order.store_id IN (?)' && $arg2 == $storeIds) {
                    return $select;
                }
            });
    ...
    public function testGetConditionsSqlIsNotFilteredAndNotNumericWebsite($operator, $value, $attribute, $checkSql)
    {
        ...
        $select->expects($this->exactly(2))
            ->method('where')
            ->willReturnCallback(function ($arg1, $arg2) use ($select, $website) {
                if ($arg1 == 'sales_order.customer_id IS NOT NULL') {
                    return $select;
                } elseif ($arg1 == 'store.website_id IN (?)' && $arg2 == $website) {
                    return $select;
                }
            });
    ...
    public function testGetConditionsSqlIsFiltered($operator, $value, $attribute, $checkSql, $customer, $customerFilter)
    {
        ...
        $select->expects($this->exactly(2))
            ->method('where')
            ->willReturnCallback(function ($arg1, $arg2) use ($select, $website, $customerFilter) {
                if ($arg1 == 'store.website_id IN (?)' && $arg2 == $website) {
                    return $select;
                } elseif ($arg1 == $customerFilter) {
                    return $select;
                }
            });
    ...
    public static function getConditionSqlDataProvider()
    {
        return [
    ...
    public static function getConditionSqlIsFilteredDataProvider()
    {
        return [
