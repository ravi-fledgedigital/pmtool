...
abstract class AbstractTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->responseMock = $this->getMockBuilder(ResponseInterface::class)
            ...
            ->addMethods(['setStatusHeader', 'setRedirect', 'representJson', 'setBody'])
            ->getMockForAbstractClass();
        $this->sessionMock = $this->getMockBuilder(Session::class)
            ->addMethods(['getEventData', 'setIsUrlNotice', 'getFormData'])
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->requestMock = $this->getMockBuilder(RequestInterface::class)
            ->addMethods(['isDispatched', 'initForward', 'setDispatched', 'isForwarded'])
            ->getMockForAbstractClass();
        ...
        $this->switcherBlockMock = $this->getMockBuilder(BlockInterface::class)
            ->onlyMethods(['toHtml'])
            ->addMethods(['setDefaultStoreName', 'setSwitchUrl'])
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
    ...
    protected function getMockForConditionsHtmlAction($className, $prefix)
    {
        ...
        $conditionMock = $this->getMockBuilder($className)
            ->setMethods(['asHtmlRecursive'])
            ->addMethods(
                ['setId', 'setType', 'setRule', 'setPrefix', 'setAttribute', 'setJsFormObject']
            )
            ->disableOriginalConstructor()
            ->getMock();
        $conditionMock
        ...
        $this->objectManagerMock
            ...
            ->method('create')
            ->willReturnCallback(function ($arg1, $arg2) use ($conditionMock, $ruleMock) {
                if ($arg1 == 'foo' && empty($arg2)) {
                    return $conditionMock;
                } elseif ($arg1 == Rule::class && empty($arg2)) {
                    return $ruleMock;
                }
            });
