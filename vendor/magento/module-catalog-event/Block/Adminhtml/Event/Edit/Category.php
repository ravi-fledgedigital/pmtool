...
class Category extends AbstractCategory
{
    ...
    public function getEventCategory(): string
    {
        ...
        $recursion = max($level);
        $category = $this->getTreeArray(null, false, $recursion);
        return $this->getEventCategoryTree($category['children']);
    }

    /**
     * Return a event category tree
     *
     * @param array $children
     * @return string
     */
    public function getEventCategoryTree($children): string
    {
        $html = "<ul>";
        foreach ($children as $category) {
            $html .= "<li id='" . $category['id'] . "' >";
            $html .= "<span>" . $category['name' ] . " (" . $category['product_count'] . ")</span>";
            if (array_key_exists("children", $category)) {
                $html .= $this->getEventCategoryTree($category["children"]);
            }
            $html .= "</li>";
        }
        $html .= "</ul>";
