...
class SubscriptionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $statementPostprocessor = $this->createMock(SubscriptionStatementPostprocessorInterface::class);
    ...
    protected function prepareTriggerTestCallMap(MockObject $triggerMock): MockObject
    {
        $triggerMock
            ->method('addStatement')
            ->willReturnCallback(function ($arg1) use ($triggerMock) {
                if ($arg1 === "SET @entity_id = (SELECT entity_identifier FROM entity_table "
                    . "WHERE entity_link_field = NEW.entity_link_field AND created_in <= UNIX_TIMESTAMP());\n"
                    . "IF (@entity_id IS NOT NULL) THEN "
                    . "INSERT IGNORE INTO test_view_cl (entity_id) values(@entity_id); "
                    . "END IF;") {
                    return $triggerMock;
                } elseif ($arg1 === "IF (@entity_id IS NOT NULL) THEN "
                    . "INSERT IGNORE INTO other_test_view_cl (entity_id) values(@entity_id); "
                    . "END IF;") {
                    return $triggerMock;
                } elseif ($arg1 === "SET @entity_id = (SELECT entity_identifier FROM entity_table "
                    . "WHERE entity_link_field = NEW.entity_link_field AND created_in <= UNIX_TIMESTAMP());\n"
                    . "IF (@entity_id IS NOT NULL) THEN "
                    . "INSERT IGNORE INTO test_view_cl (entity_id) values(@entity_id); "
                    . "END IF;") {
                    ...
                    . "WHERE entity_link_field = OLD.entity_link_field AND created_in <= UNIX_TIMESTAMP());\n"
                    . "IF (@entity_id IS NOT NULL) THEN "
                    . "INSERT IGNORE INTO test_view_cl (entity_id) values(@entity_id); "
                    . "END IF;") {
                    return $triggerMock;
