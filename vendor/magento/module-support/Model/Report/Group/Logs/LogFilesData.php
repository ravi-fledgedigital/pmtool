...
class LogFilesData
{
    ...
    /**
     * Regular expression for log file entries like [2023-03-22T09:28:07.591068+00:00] main.CRITICAL: ...
     */
    private const LOG_RECORDS_REG_EXP = '~\[([\d-]+)T([\d:]+)[\d:\+\.]+\].*?:\s(.*)~';
    ...
    public function getLogFilesData()
    {
        ...
        foreach ($entry as $file) {
            ...
            if ($this->directory->isReadable($file)) {
                ...
                $matched = (int)preg_match_all(self::LOG_RECORDS_REG_EXP, $fileContent);
                $logEntriesNumber += $matched;
                // Collect system log messages
                if ($file == self::SYSTEM_LOG_FILE
                    && preg_match_all(self::LOG_RECORDS_REG_EXP, $fileContent, $matches)
                ) {
    ...
    protected function getSystemLogMessages($matches)
    {
        foreach ($matches[0] as $line) {
            preg_match(self::LOG_RECORDS_REG_EXP.'im', $line, $matches);
        ...
        foreach ($lines as $line) {
            ...
            if ($this->exceptionStarted === true && $this->exceptionMessage === ''
                && (preg_match('~exception (.+)$~im', $line, $matches)
                    ...
                    ||
                    preg_match('~.*:\s(.*Exception.+)$~im', $line, $matches)
                )
            ) {
            ...
            if ($this->exceptionEnded !== true && $this->exceptionMessage !== ''
                ...
                && !preg_match('~.*:\s(.*Exception.+)$~im', $line)
            ) {
