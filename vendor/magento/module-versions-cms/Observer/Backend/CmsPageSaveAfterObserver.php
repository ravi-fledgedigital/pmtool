...
class CmsPageSaveAfterObserver implements ObserverInterface
{
    ...
    private function createNewNode(
        ...
        if ($parentNode !== null) {
            ...
            $xPath = $newNode->getXpath();
            $alreadyExistedNode = $this->isNodeExistsInStoreView(
                $requestUrl,
                $pageData[HierarchyNode::SCOPE],
                $pageData[HierarchyNode::SCOPE_ID],
                (int) $pageData['page_id']
            );
        }
        if (!$alreadyExistedNode) {
            $newNode->addData(
                $pageData
            )->setParentNodeId(
                $newNode->getId()
    ...
    /**
     * Check if same node exist for the same store and scope
     *
     * @param string $requestUrl
     * @param string $scope
     * @param int $scopeId
     ...
     * @return bool
     */
    private function isNodeExistsInStoreView(
        string $requestUrl,
        string $scope,
        int $scopeId,
        int $currentPageId
        ...
        $nodeCollection->addFieldToFilter('request_url', $requestUrl)
            ...
            ->addFieldToFilter('scope_id', $scopeId)
            ->addFieldToFilter('main_table.page_id', ['eq' => $currentPageId]);
