...
class Chooser extends \Magento\Backend\Block\Template
{
     ...
     * @return void
     */
    public function getCMSHierarchyCategory()
    {
        $hierarchyNodes = $this->getNodes();

        if (count($hierarchyNodes) > 0) {
            $tree = $this->buildTree($hierarchyNodes);
            return $this->getCMSHierarchyTree($tree);
        }
    }
     ...
     * @return string
     */
    public function getCMSHierarchyTree($tree): string
    {
        $html = "<ul>";
        foreach ($tree as $category) {
            $html .= "<li id='" . $category['node_id'] . "' >";
            $html .= "<span>" . $category['label'] . "</span>";
            if (array_key_exists("children", $category)) {
                $html .= $this->getCMSHierarchyTree($category["children"]);
            }
            $html .= "</li>";
        }
        $html .= "</ul>";
    ...
    /**
     * Build the tree with children recursively
     *
