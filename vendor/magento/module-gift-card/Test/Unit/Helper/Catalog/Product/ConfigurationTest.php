...
class ConfigurationTest extends TestCase
{
    ...
    private function prepareCustomOptions(MockObject $itemMock, array ...$data): void
    {
        ...
        $itemMock
            ...
            ->willReturnCallback(function ($itemMockWithArgs) use ($itemMockWillReturnArgs) {
                if ($itemMockWithArgs) {
                    static $callCount = 0;
                    $returnValue = $itemMockWillReturnArgs[$callCount] ?? null;
                    $callCount++;
                    return $returnValue;
                }
            });
        $this->escaper
            ->method('escapeHtml')
            ->willReturnCallback(function ($escaperWithArgs) use ($escaperWillReturnArgs) {
                if ($escaperWithArgs) {
                    static $callCount = 0;
                    $returnValue = $escaperWillReturnArgs[$callCount] ?? null;
                    $callCount++;
                    return $returnValue;
                }
            });
